...
def process_single_animal(df: pd.DataFrame, body_parts: List[str], 
                         interpolation_limit: int = 5) -> pd.DataFrame:
    """
    ...
    df = interpolate_missing_data(df, body_parts, limit=interpolation_limit)
...
def process_paired_animals(df: pd.DataFrame, body_parts: List[str],
    ...
    df0 = interpolate_missing_data(df0, body_parts, limit=interpolation_limit)
    df1 = interpolate_missing_data(df1, body_parts, limit=interpolation_limit)
...
def main(snakemake):
    ...
    # Load metadata to determine single vs paired
    ...
    # Process based on number of animals
    if unique_tracks == 1:
        processed_df = process_single_animal(df, body_parts, interpolation_limit)
        swap_report = {'swap_events': [], 'mode': 'single_animal'}
    elif unique_tracks == 2:
        processed_df, swap_report = process_paired_animals(df, body_parts, interpolation_limit)
        swap_report['mode'] = 'paired_animals'
    else:
        raise ValueError(f"Unsupported number of tracks: {unique_tracks}")
    
    # Save outputs
    print("\nSaving outputs...")
    processed_df.to_csv(snakemake.output.processed_csv, index=False)
    print(f"  ✓ Processed poses: {snakemake.output.processed_csv}")
    
    with open(snakemake.output.swap_report, 'w') as f:
        json.dump(swap_report, f, indent=2)
    print(f"  ✓ Swap report: {snakemake.output.swap_report}")
